...
#     BENCH_FILE_DUMMY="${ROOFLINE_DIR}/${element}_dummy_loop_more_calcs.txt"
...
AVG_INTR="${ROOFLINE_DIR}/intrinsics_dummy_loop_more_calcs.txt"
AVG_OMP="${ROOFLINE_DIR}/omp_dummy_loop_more_calcs.txt"
AVG_VC="${ROOFLINE_DIR}/vc_dummy_loop_more_calcs.txt"
AVG_SIMPLE_DUMMY="${ROOFLINE_DIR}/simple_dummy_loop_more_calcs.txt"
...
gnuplot << EOF
    ...
    set output "$ROOFLINE_DIR/more_calcs_roofline_model.png"
    ...
    set output "$ROOFLINE_DIR/more_calcs_benchmark.png"
    
    set title "Benchmark for more calculations" font ",16"
    set xlabel "number of extra operations"
    ...
    plot \
        "$AVG_SIMPLE_DUMMY" using 1:2 with linespoints title "Simple dummy" lw 2 pt 7 lc rgb "red", \
        "$AVG_INTR" using 1:2 with linespoints title "intr" lw 2 pt 9 lc rgb "blue", \
        "$AVG_VC" using 1:2 with linespoints title "vc" lw 2 pt 5 lc rgb "green", \
        "$AVG_OMP" using 1:2 with linespoints title "omp" lw 2 pt 11 lc rgb "black"
EOF